...
export class PrepsMessageTokenComponentsPage {
  createButton = element(by.id('jh-create-entity'));
  deleteButtons = element.all(by.css('gha-preps-message-token div table .btn-danger'));
  title = element.all(by.css('gha-preps-message-token div h2#page-heading span')).first();
...
export class PrepsMessageTokenUpdatePage {
  pageTitle = element(by.id('gha-preps-message-token-heading'));
...
export class PrepsMessageTokenDeleteDialog {
  private dialogTitle = element(by.id('gha-delete-prepsMessageToken-heading'));
